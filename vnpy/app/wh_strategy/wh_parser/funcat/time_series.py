...
def get_bars():
    ...
    # if data_backend.skip_suspended and parser_str_to_time(str(bars["datetime"][-1])) != current_date:
    #     return np.array([])
...
class MarketDataSeries(NumericSeries):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, six.string_types):
            ...
            assert period > 0
            # 因为是行情数据，所以需要动态更新
            time_series = self.__class__(dynamic_update=False, **self.extra_create_kwargs)
            return time_series
